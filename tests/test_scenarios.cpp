...
TEST_CASE("Scenario: full pipeline ModDB + passives", "[scenario]") {
  ...
  // Passive mod (AD: Base=0.1*final) folds into mod_db, so:
  // final = (80 + 0.1*final) * 1.2 * 1.5 = (80 + 0.1*final) * 1.8
  // final = 144 + 0.18*final → final = 144/0.82 ≈ 175.6098
  Stats base = champ.getBaseStats();
  REQUIRE(base[std::to_underlying(Stat::AD)] == Catch::Approx(144.0));
  Stats result = champ.evaluateChampion(0.0001);
  REQUIRE(result[std::to_underlying(Stat::AD)] ==
          Catch::Approx(175.6098).epsilon(0.001));
}
...
TEST_CASE("Scenario: one-shot passive fires once then permanent remains",
  ...
  champ.mod_db.add(Stat::MaxHP, ModType::Base, 1000.0, Source{"Base", ""});
  champ.addPassive(
      factory().make(PassiveId::Test11, [](const Stats &, const Stats &, Type) {
        return Champion::PassiveResult{
            {{Stat::MaxHP, ModType::Base, 100.0, {}}},
            true}; // permanent
      }));
  champ.addPassive(
      factory().make(PassiveId::Test12, [](const Stats &, const Stats &, Type) {
        return Champion::PassiveResult{
            {{Stat::MaxHP, ModType::Base, 500.0, {}}},
            false}; // one-shot
      }));
  ...
  REQUIRE(first[std::to_underlying(Stat::MaxHP)] == Catch::Approx(1600.0));
  // one-shot consumed; only perm remains
  REQUIRE(champ.passives.size() == 1);

  // second step: only perm: 1000 + 100 = 1100
  Stats second = champ.applyPassives(base, base, 1.0);
  REQUIRE(second[std::to_underlying(Stat::MaxHP)] == Catch::Approx(1100.0));
  REQUIRE(champ.passives.size() == 1);
}
...
TEST_CASE("Scenario: complex pipeline with mod_db + perm + one-shot + temp",
  ...
  // one-shot: +200 HP
  champ.addPassive(
      factory().make(PassiveId::Test17, [](const Stats &, const Stats &, Type) {
        return Champion::PassiveResult{
  ...
  champ.addPassive(
      factory().make(PassiveId::Test18, [](const Stats &, const Stats &, Type) {
        return Champion::PassiveResult{{{Stat::AD, ModType::Base, 50.0, {}}},
                                       true};
      }));

  Stats r = champ.evaluateChampion(0.0001);
  // HP: (1000 + 200) * 1 * 1 = 1200 (no Inc/More on HP)
  REQUIRE(r[std::to_underlying(Stat::MaxHP)] == Catch::Approx(1200.0));
  // AD: passive mods (0.05*HP + 50) fold into mod_db as Base, so:
  // sum(AD) = 80 + 0.05*1200 + 50 = 190; final = 190 * 1.2 * 1.5 = 342
  REQUIRE(r[std::to_underlying(Stat::AD)] ==
          Catch::Approx(342.0).epsilon(0.01));
...
TEST_CASE("Scenario: two one-shot passives fire in single evaluateChampion",
  ...
  champ.mod_db.add(Stat::AD, ModType::Base, 50.0, Source{"Base", ""});
  champ.addPassive(
      factory().make(PassiveId::Test21, [](const Stats &, const Stats &, Type) {
        return Champion::PassiveResult{{{Stat::AD, ModType::Base, 20.0, {}}},
                                       false};
      }));
  champ.addPassive(
      factory().make(PassiveId::Test22, [](const Stats &, const Stats &, Type) {
        return Champion::PassiveResult{{{Stat::AD, ModType::Base, 30.0, {}}},
                                       false};
      }));
  Stats r = champ.evaluateChampion();
...
TEST_CASE("Scenario: large negative passive brings stat below zero",
  ...
  champ.mod_db.add(Stat::AD, ModType::Base, 50.0, Source{"Base", ""});
  champ.addPassive(
      factory().make(PassiveId::Test23, [](const Stats &, const Stats &, Type) {
        return Champion::PassiveResult{{{Stat::AD, ModType::Base, -75.0, {}}},
                                       true};
      }));
  Stats r = champ.evaluateChampion();
...
TEST_CASE("Scenario: passive chain AD→AP→HP converges", "[scenario]") {
  ...
  // (gains from HP)
  REQUIRE(r[std::to_underlying(Stat::MaxHP)] > 1000.0);
...
TEST_CASE("Scenario: temp passive re-inserted after expiry in simulation",
  ...
  // initial burn at t=0
  champ.addPassive(factory().make(PassiveId::Test27, make_burn(0.0)));
  // t=1: alive
  ...
  champ.addPassive(factory().make(PassiveId::Test28, make_burn(2.0)));
  // t=3: alive (3 - 2 = 1 < 2)
...
TEST_CASE("Scenario: evaluateChampion with all passive types and cross-stat "
  ...
  champ.mod_db.add(Stat::MaxHP, ModType::Base, 1500.0, Source{"Base", ""});
  champ.mod_db.add(Stat::AP, ModType::Base, 100.0, Source{"Base", ""});

  // permanent: AD += 2% of final HP
  champ.addPassive(
      factory().make(PassiveId::Test29,
                     [](const Stats &, const Stats &final, Type) {
                       return Champion::PassiveResult{
                           {{Stat::AD,
                             ModType::Base,
                             final[std::to_underlying(Stat::MaxHP)] * 0.02,
                             {}}},
                           true};
                     }));
  // one-shot: +300 HP
  champ.addPassive(
      factory().make(PassiveId::Test30, [](const Stats &, const Stats &, Type) {
        return Champion::PassiveResult{
            {{Stat::MaxHP, ModType::Base, 300.0, {}}},
            false};
      }));
  // temp: +40 AP, alive=true (stays after evaluation)
  champ.addPassive(
      factory().make(PassiveId::Test31, [](const Stats &, const Stats &, Type) {
        return Champion::PassiveResult{{{Stat::AP, ModType::Base, 40.0, {}}},
                                       true};
      }));

  Stats r = champ.evaluateChampion(0.0001);
  // HP: (1500 + 300) * 1 * 1 = 1800 (no Inc/More on HP)
  REQUIRE(r[std::to_underlying(Stat::MaxHP)] == Catch::Approx(1800.0));
  // AP: (100 + 40) * 1 * 1 = 140 (no Inc/More on AP)
  REQUIRE(r[std::to_underlying(Stat::AP)] == Catch::Approx(140.0));
  // AD: passive mod (0.02*HP=36) folds into mod_db as Base, so:
  // sum(AD) = 80 + 36 = 116; final = 116 * 1.2 * 1.5 = 208.8
  REQUIRE(r[std::to_underlying(Stat::AD)] ==
          Catch::Approx(208.8).epsilon(0.01));
...
TEST_CASE("Scenario: non-converging two-passive system throws", "[scenario]") {
  ...
  champ.addPassive(
      factory().make(PassiveId::Test32,
                     [](const Stats &, const Stats &final, Type) {
                       return Champion::PassiveResult{
                           {{Stat::AD,
                             ModType::Base,
                             final[std::to_underlying(Stat::MaxHP)] * 2.0,
                             {}}},
                           true};
                     }));
  // HP += 2 * final AD (runaway)
  champ.addPassive(
      factory().make(PassiveId::Test33,
                     [](const Stats &, const Stats &final, Type) {
                       return Champion::PassiveResult{
                           {{Stat::MaxHP,
                             ModType::Base,
