...
TEST_CASE("post_mitigation_damage is linear in raw damage", "[post_mitigation_damage]") {
  const Type armor = 75.0;
  const Type factor = 100.0 / (100.0 + armor);
  REQUIRE(post_mitigation_damage(1.0, armor) == Catch::Approx(factor).epsilon(0.001));
  REQUIRE(post_mitigation_damage(10.0, armor) == Catch::Approx(10.0 * factor).epsilon(0.001));
  REQUIRE(post_mitigation_damage(1000.0, armor) == Catch::Approx(1000.0 * factor).epsilon(0.001));
}
...
TEST_CASE("post_mitigation_damage is symmetric in armor sign for small values",
  ...
  REQUIRE(post_mitigation_damage(100.0, 1.0) == Catch::Approx(99.0099).epsilon(0.001));
  REQUIRE(post_mitigation_damage(100.0, -1.0) == Catch::Approx(101.0101).epsilon(0.001));
}

TEST_CASE("post_mitigation_damage halves at armor=100", "[post_mitigation_damage]") {
  REQUIRE(post_mitigation_damage(100.0, 100.0) == Catch::Approx(50.0));
}

TEST_CASE("post_mitigation_damage at armor=300 is 25%", "[post_mitigation_damage]") {
  // 100 * 100/(100+300) = 25
  REQUIRE(post_mitigation_damage(100.0, 300.0) == Catch::Approx(25.0));
}
...
TEST_CASE("post_mitigation_damage tiny raw damage with huge armor is tiny",
          "[post_mitigation_damage]") {
  REQUIRE(post_mitigation_damage(0.001, 10000.0) == Catch::Approx(0.0000099).epsilon(0.01));
}